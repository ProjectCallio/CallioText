 ...
 * 获得一个路径的最近的概念父亲节点。
 */
function slate_concept_father_path<RootType = ConceptNode>(root: RootType & Slate.Node, path: number[]): (Slate.Node & ConceptNode) | undefined {
    let candicate: (Slate.Node & ConceptNode) | undefined = undefined
    for(let [nd,_] of Slate.Node.ancestors(root, path)){
...
function slate_concept_father<RootType = ConceptNode>(root: RootType & Slate.Node, node: Slate.Node & ConceptNode): (Slate.Node & ConceptNode) | undefined {
    return slate_concept_father_path<RootType>(root, slate_concept_node2path(root, node))
}
