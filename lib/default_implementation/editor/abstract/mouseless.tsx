...
const SPACE: SpaceDefinition = {
    ...
    onStart: (from)=> onStartMaker(useCurEditor.current)(from) ?? NO_ACTION, 
...
function onStartMaker(get_editor: ()=>EditorComponent | undefined): (last?: NodeName)=> NodeName | undefined{
    return (last?: NodeName): NodeName | undefined => {
        ...
        const now_node = editor.get_cur_concept_node()
        if(!now_node){
            return undefined
        }
        ...
        const now_idx = now_node.idx

        if(last != undefined){
            const [old_nodeidx, _] = JSON.parse(last)
